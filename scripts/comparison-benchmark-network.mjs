...
globalThis.fetch = async (input, init) => {
  ...
  appendFileSync(process.env.WORKSTATION_TEST_PROVIDER_LOG, JSON.stringify({ symbol, timeframe, metric: timeframe === "1Day" && Date.parse(url.searchParams.get("end")) - Date.parse(url.searchParams.get("start")) > 400 * 86400000, from: url.searchParams.get("start"), to: url.searchParams.get("end") }) + "\n");
  await wait(50, undefined, { signal: init?.signal });
  ...
  if (symbol === "MU" || (symbol === "NVDA" && nvda)) {
    ...
    const origin = step >= 86400 ? 14.5 * 3600 + (step === 604800 ? 4 * 86400 : 0) : 0;
    for (let t = Math.ceil((from - origin) / step) * step + origin; t <= to; t += step) {
      const date = new Date(t * 1000); if (["2025-01-01", "2025-01-09", "2025-01-20", "2025-02-17", "2025-04-18", "2025-05-26", "2025-06-19", "2025-07-04", "2025-09-01", "2025-11-27", "2025-12-25", "2026-01-01", "2026-01-19", "2026-02-16", "2026-04-03", "2026-05-25", "2026-06-19", "2026-07-03", "2026-09-07"].includes(date.toISOString().slice(0,10))) continue; if ([0, 6].includes(date.getUTCDay())) continue;
