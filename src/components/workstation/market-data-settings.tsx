...
type Status = { enabled: boolean; preparation: boolean; usage: null | { cacheBytes: number; databaseBytes: number; cacheLimit: number; databaseLimit: number }; counts: Record<string, number>; issues: { key: string; symbol: string; timeframe: string; start: string; end: string; status: string; lastError: string | null }[] };
export function MarketDataSettings() {
  ...
  const working = busy || !!status?.counts.pending || !!status?.counts.running;
  ...
  async function action(action: "plan" | "run" | "retry") {
  ...
  return <div className="space-y-4 text-sm" aria-busy={busy}>
    ...
    {status && <>
      ...
      <div className="flex flex-wrap gap-2">{([['plan', 'Queue all trade windows'], ['run', 'Run / resume batch'], ['retry', 'Retry failed windows']] as const).map(([key, label]) => <button className="rounded-md border px-3 py-2 disabled:opacity-50" key={key} disabled={busy || !status.preparation || !checked} onClick={() => void action(key)}>{label}</button>)}<button className="rounded-md border px-3 py-2" disabled={busy} onClick={() => void refresh().catch(e => setError(e.message))}>Refresh status</button></div>
