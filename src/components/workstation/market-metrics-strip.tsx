...
export function useMarketMetrics(adapter: WorkstationAdapter, trade: Trade | undefined) {
  ...
  const key = `${trade?.id}:${trade?.timeInterpretationVersion}`;
  useEffect(() => {
...
export function MarketMetricsStrip({ value, error, retry }: ReturnType<typeof useMarketMetrics>) {
  ...
  return <div className="ws-market-metrics" aria-label="Pre-trade metrics">
    ...
    {([["ADR% · 14", display.adr, "percent"], ["ATR% · 14", display.atr, "percent"], ["Avg dollar volume · 20", display.dollarVolume, "money"], ["Market cap · estimated", display.marketCap, "money"]] as const).map(([name, metric, kind]) => <span key={name} title={metric.reason ?? (name.startsWith("ATR") && display.atrSessions < 250 ? `Wilder ATR initialized with ${display.atrSessions} available sessions` : name.startsWith("Market") ? `Estimated from SEC shares dated ${display.sharesDate}, filed ${display.sharesFiled}; same-session filings excluded; adjusted for intervening splits` : display.provider)}><small>{name}</small><b>{value ? formatMetric(metric, kind, display.currency) : "—"}</b></span>)}
...
export function TradeMarketMetrics({ adapter, trade, onValue }: { adapter: WorkstationAdapter; trade: Trade; onValue: (key: string, value: MarketMetrics | undefined) => void }) {
  const result = useMarketMetrics(adapter, trade), key = `${trade.id}:${trade.timeInterpretationVersion}`;
