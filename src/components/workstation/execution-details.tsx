...
export function ExecutionDetails({ diagnostic: d, history, onClose }: { diagnostic: ExecutionDiagnostic; history: CandleResult; onClose(): void }) {
  ...
  return <aside ref={ref} className="ws-execution-details" role="dialog" aria-label="Execution details" onKeyDown={event => { if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); onClose(); } }}>
    ...
    <dl><dt>Stored execution time</dt><dd>{utc(d.execution.time)}</dd><dt>Candle interval</dt><dd>{d.period ? `${utc(d.period.start)} – ${utc(d.period.end)} (end exclusive)` : "No containing candle in loaded history"}</dd>
    ...
    {d.status === "price-outside" && <p>The recorded price is outside this candle. Timezone interpretation, session coverage, feed differences, or price adjustments may explain it. The execution price and time are preserved.</p>}
