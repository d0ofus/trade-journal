...
type Props = { onImage: (section: ReviewSectionKey) => void; getMetrics?: () => MarketMetrics | undefined; trade: Trade; document: TradeDocument; onChange: (update: (review: Review) => Review) => void; status: string; error: string; retry: () => void; reload: () => void; onSaveNext: () => void; saveNextShortcut: string; onEvidence: (section?: ReviewSectionKey) => void; onComparePeers: (selection: PeerGroupSelection) => void; onNotionExport: () => void; preferences: WorkspacePreferences; onPreferences: (update: Partial<WorkspacePreferences>) => void; replay: number | null; mode: "demo" | "application"; notify: (message: string) => void };
export function ReviewEditor(p: Props) {
  ...
  return <div className="ws-journal-content">
    ...
    <NotionReviewEditor trade={p.trade} document={p.document} onChange={p.onChange} onEvidence={p.onEvidence} onImage={p.onImage} onComparePeers={p.onComparePeers} mode={p.mode} />
    ...
    <SectionAttachments section="previousReview" document={p.document} onChange={p.onChange} onEvidence={p.onEvidence} onImage={p.onImage} readOnly={p.trade.stale} />
    </details>
