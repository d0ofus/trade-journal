...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = body.action === "pilot" ? { ...await queueCandlePreparation(true), ...await runCandlePreparation(200_000, true) } : body.action === "plan" ? await queueCandlePreparation(false, body.recheck === true) : body.action === "retry" ? await retryCandlePreparation() : await runCandlePreparation();
