...
export async function GET(request: NextRequest) {
  ...
  try {
    const loaded = await loadWorkstationCandles({ symbol, timeframe: timeframe as Interval, range: { from, to }, limit: limit + 1, signal: request.signal, identity: params.get("identity"), session: sessionMode as "regular" | "extended" });
