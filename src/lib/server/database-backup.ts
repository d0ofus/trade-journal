...
export async function databaseBackup(backupId?: string) {
  ...
  const [
    ...
    evidenceAssetReferences,
    evidenceMaintenanceStates,
    latestBackupRelevantUpdateAt,
  ] = await prisma.$transaction(
    async (tx) => {
      if (backupId) await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext('workstation-evidence-gc'))`;
      const rows = await Promise.all([
        ...
        tx.executionTimeInterpretation.findMany({ orderBy: { id: "asc" } }),
        tx.accountExecutionTimePolicy.findMany({ orderBy: { id: "asc" } }),
        tx.executionTimePolicyApplication.findMany({ orderBy: { key: "asc" } }),
        ...
        tx.evidenceAsset.findMany({ where: { state: "ready" }, orderBy: { id: "asc" } }),
        tx.evidenceUploadSession.findMany({ where: { state: "ready" }, orderBy: { id: "asc" } }),
        tx.evidenceAssetReference.findMany({ where: { kind: { not: "backup" } }, orderBy: { assetId: "asc" } }),
        tx.evidenceMaintenanceState.findMany({ orderBy: { key: "asc" } }),
        getLatestBackupRelevantUpdateAt(tx),
      ] as const);
      if (backupId) for (const asset of await tx.evidenceAsset.findMany({ where: { state: "ready" }, select: { id: true } })) {
        await tx.evidenceAssetReference.create({ data: { assetId: asset.id, kind: "backup", key: backupId, expiresAt: new Date(Date.now() + 86_400_000) } });
        await tx.evidenceAsset.update({ where: { id: asset.id }, data: { unreferencedAt: null } });
      }
      return rows;
    },
    {
  ...
  const payload = {
    ...
    ...tablePayload,
    assets: {
      r2Evidence: { complete: !evidenceAssets.length, requiresOriginals: evidenceAssets.length, warning: evidenceAssets.length ? "Database metadata only: incomplete without the R2 originals in a complete evidence backup bundle." : null },
