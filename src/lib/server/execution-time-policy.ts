...
import { TIME_INTERPRETATION_VERSION } from "@/lib/workstation/timestamp-interpretation";
...
function inspect(policy: Pick<AccountExecutionTimePolicy, "accountId" | "timezone">, batch: Batch) {
  ...
  else if (!["flex-trades", "executions"].includes(batch.fileType) || batch.parserVersion !== "2026-06-25-workstation-uplift") reason = "Unsupported report format or parser version; individual confirmation is required.";
  else if (!content || !batch.rawSha256 || batch.rawSha256 !== batch.rawArtifact?.rawSha256 || hash(content) !== batch.rawSha256) reason = "Archived source is missing or its fingerprint changed.";
  else if (Buffer.byteLength(content) > 16 * 1024 * 1024 || executions.length > 20000) reason = "Report exceeds the bounded interpretation limit.";
  let rows: ReturnType<typeof inspectTimestampRows> = [];
  if (!reason) { try { rows = inspectTimestampRows(content!, executions, policy.timezone); } catch { reason = "Archived report could not be interpreted."; } }
  const sourceHash = batch.rawSha256 ?? "", fingerprint = hash(JSON.stringify({ sourceHash, artifactHash: batch.rawArtifact?.rawSha256, parserVersion: batch.parserVersion, timezone: policy.timezone, accountId: policy.accountId, version: TIME_INTERPRETATION_VERSION, override: batch.timeInterpretation?.revision, executions: executions.map(e => [e.id, e.dedupeKey, e.executedAt.toISOString(), e.price, e.quantity, e.side]), rows }));
  return { batchId: batch.id, filename: batch.filename, sourceHash, parserVersion: batch.parserVersion, fingerprint, status: batch.timeInterpretation ? "exception" : reason || rows.some(r => r.interpretedTime === null) ? "unresolved" : "ready", reason, rows, eligible: rows.filter(r => r.interpretedTime !== null).length, total: executions.length };
}
...
export async function previewAccountTimePolicy(accountId: string, db: Reader = prisma) {
  ...
  const rows = (await reports(accountId, db)).map(b => inspect({ accountId, timezone: "America/New_York" }, b));
  return { accountId, account: account.ibkrAccount, timezone: "America/New_York", revision: existing?.revision ?? 0, active: existing?.active ?? false, fingerprint: hash(JSON.stringify(rows.map(r => r.fingerprint))), reports: rows.map(r => ({ batchId: r.batchId, filename: r.filename, status: r.status, reason: r.reason, eligible: r.eligible, total: r.total })), eligible: rows.reduce((n, r) => n + r.eligible, 0), total: rows.reduce((n, r) => n + r.total, 0) };
}
export async function saveAccountTimePolicy(accountId: string, expectedRevision: number, active: boolean, fingerprint?: string) {
  return prisma.$transaction(async tx => {
    await tx.$queryRaw(Prisma.sql`SELECT pg_advisory_xact_lock(hashtextextended(${`account-time-policy:${accountId}`},0))::text`);
    const preview = await previewAccountTimePolicy(accountId, tx);
    ...
    const data = { active, revision: expectedRevision + 1, timezone: "America/New_York", normalizerVersion: TIME_INTERPRETATION_VERSION, basis: "user-confirmed", confirmedAt: new Date() };
...
export async function accountTimePolicyStatus() {
  ...
  return accounts.map((a, index) => { const p = policies.find(p => p.accountId === a.id); const latest = [...new Map((p?.applications.filter(r => r.policyRevision === p.revision) ?? []).slice().reverse().map(r => [r.importBatchId, r])).values()]; return { ...a, policy: p ? { id: p.id, active: p.active, revision: p.revision, timezone: p.timezone, pending: Math.max(0, counts[index] - latest.length), ready: latest.filter(r => r.status === "ready").length, unresolved: latest.filter(r => r.status === "unresolved").length, exceptions: latest.filter(r => r.status === "exception").length } : null }; });
}
...
export function applyAccountTimePolicy(execution: Execution, source: Source, policy?: AccountExecutionTimePolicy, application?: ExecutionTimePolicyApplication): Execution {
  ...
  const unresolved = (status: "pending" | "stale" | "unresolved") => ({ ...execution, provenance: { ...execution.provenance!, interpretationStatus: status, interpretationVersion: `policy:${policy.id}:${policy.revision}`, confirmationBasis: "user-confirmed account default" } });
  ...
  if (application.status === "exception" || !source) return unresolved("unresolved");
