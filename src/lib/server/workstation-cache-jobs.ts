...
import { isOptionTrade } from "@/lib/workstation/preparation-eligibility";
...
export function planCandlePreparation(trades: Trade[], now = Date.now() / 1000): PreparationWindow[] {
  ...
  for (const trade of trades.filter(t => !isOptionTrade(t))) for (const timeframe of preparationIntervals) {
...
export async function queueCandlePreparation(pilot = false, recheck = false, refreshRecent = false) {
  ...
  const allTrades = (await listWorkstationTrades()).filter(t => !isOptionTrade(t) && !t.executions.some(e => e.provenance?.interpretationStatus === "pending"));
  ...
  const keys = windows.map(w => cacheHash(`${w.symbol}:${w.timeframe}:${candleIdentity(policy.cacheSource, w.timeframe, w.session)}:${w.range.from}:${w.range.to}`));
  await skipUnselectedPreparation();
  if (!pilot) await prisma.workstationCandleJob.updateMany({ where: { key: { notIn: keys }, OR: [{ status: { in: ["pending", "failed", "unavailable"] } }, { status: "running", leaseUntil: { lt: new Date() } }] }, data: { status: "skipped", leaseUntil: null, leaseToken: null, lastError: "Superseded by the current trade preload ranges. Cached candles are retained." } });
  ...
  for (let offset = 0; offset < windows.length; offset += 200) {
    ...
    await prisma.workstationCandleJob.updateMany({ where: { key: { in: batch.map(w => w.key) }, status: { in: recheck ? ["skipped", "done"] : ["skipped"] } }, data: { status: "pending", attempts: 0, availableAt: new Date(), lastError: null } });
    if (refreshRecent) await prisma.workstationCandleJob.updateMany({ where: { key: { in: batch.map(w => w.key) }, status: "done", end: { gt: new Date(Date.now() - 7 * 86400_000) }, updatedAt: { lt: new Date(Date.now() - 86400_000) } }, data: { status: "pending", availableAt: new Date() } });
...
export async function runCandlePreparation(durationMs = 200_000, pilot = false) {
  ...
  if (timing.pending) return { processed: 0, advanced: timing.prepared, retryAfterMs: 1000, paused: `${timing.pending} reports await timestamp preparation.` };
  await skipUnselectedPreparation();
  const policy = workstationCandlePolicy();
  ...
  if (!slot) return { processed: 0, retryAfterMs: 5000, paused: "Two preparation runners are already active." };
  const deadline = Date.now() + Math.min(200_000, Math.max(0, durationMs)); let processed = 0, advanced = 0, providerRequests = 0;
  try {
    while (Date.now() < deadline && (pilot ? cacheEnabled() : preparationEnabled())) {
      if (!(await cacheBudgetAvailable())) return { processed, advanced, providerRequests, paused: "Storage budget reached. Existing candles remain available." };
      ...
      try {
        ...
        if (result.cache?.progressed) advanced++;
        providerRequests += result.cache?.timings?.providerRequestCount ?? 0;
        ...
        if (result.cache?.persistencePaused) return { processed, advanced, providerRequests, paused: "Storage budget reached. Existing candles remain available." };
      } catch (error) {
    ...
    const next = await prisma.workstationCandleJob.aggregate({ where: { status: "pending", ...(pilot ? { priority: 0 } : {}) }, _min: { availableAt: true } });
    return { processed, advanced, providerRequests, nextAvailableAt: next._min.availableAt, paused: null };
  } finally { await releaseCacheLease(slot); }
}
export async function skipUnselectedPreparation() {
  const instruments = await prisma.instrument.findMany({ select: { symbol: true, assetType: true } });
  const optionSymbols = instruments.filter(isOptionTrade).map(i => i.symbol);
  await prisma.workstationCandleJob.updateMany({ where: { symbol: { in: optionSymbols }, OR: [{ status: { in: ["pending", "failed", "unavailable"] } }, { status: "running", leaseUntil: { lt: new Date() } }] }, data: { status: "skipped", leaseUntil: null, leaseToken: null, lastError: "Option contracts are excluded from automatic candle preparation." } });
...
export async function recoverCandlePreparation() {
  ...
  await skipUnselectedPreparation();
  // Refresh only currently selected ranges; retired sessions and options stay retired.
