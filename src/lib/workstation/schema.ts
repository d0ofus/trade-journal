...
export const workstationDocumentSchema = z.object({
  ...
  evidence: z.array(z.object({ id: z.string().min(1).max(200), name: z.string().max(240), image: z.string().max(6000000).regex(/^data:image\/png;base64,[A-Za-z0-9+/]+=*$/), time: z.number().finite().nonnegative(), revision: z.number().int().nonnegative(), timeframe: z.string().max(40) })).max(30),
