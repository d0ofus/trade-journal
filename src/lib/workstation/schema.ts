...
export const workstationDocumentSchema = z.object({
  ...
  drawings: z.array(z.object({ id: z.string().min(1).max(200), tool: z.enum(drawingTools).exclude(["cursor"]), points: z.array(point).min(1).max(4), text: z.string().max(500), color: z.string().regex(/^#[a-fA-F0-9]{6}$/), width: z.number().min(.5).max(4), dashed: z.boolean(), showDefaultLabel: z.boolean().optional(), locked: z.boolean(), hidden: z.boolean(), panel: z.string().max(100).nullable(), createdAt: z.number().finite().nonnegative() }).superRefine((d, ctx) => { if (["trend", "arrow", "zone", "measure", "long", "short"].includes(d.tool) && d.points.length < 2) ctx.addIssue({ code: "custom", message: "This drawing needs two anchors" }); })).max(500),
