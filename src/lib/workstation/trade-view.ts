...
export const tradeViewSchema = z.object({
  version: z.literal(1),
  panels: z.array(z.object({ id: z.string().regex(/^chart-[1-4]$/), interval: z.enum(intervals), session: z.enum(["auto", "regular", "extended"]), range: range.nullable(), benchmark: z.enum(["off", "SPY", "QQQ"]).optional(), beforeEntry: z.boolean().optional() }).strict()).min(1).max(4).refine(p => new Set(p.map(v => v.id)).size === p.length),
