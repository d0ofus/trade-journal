...
export function rebaseComparison(primary: Candle[], benchmark: Candle[], range: { from: number; to: number } | null): Comparison {
  ...
  const factor = anchor.open / base.open;
  return { anchor: anchor.time, primaryOpen: anchor.open, benchmarkOpen: base.open, originals,
    candles: primary.flatMap(p => { const c = originals.get(p.time); return c ? [{ ...c, open: c.open * factor, high: c.high * factor, low: c.low * factor, close: c.close * factor }] : []; }) };
}
