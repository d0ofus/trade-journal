...
test("overlapping daily executions stay readable below the compact price readout", () => {
  ...
  const executions = paintChart(ctx, { width: 420, height: 310, plotWidth: 360, plotHeight: 285, x: () => 190, y: price => price > trade.entry ? 12 : 200, drawings: [], trade, candles, interval: "1d", labels: "labels", selected: null, selectedExecution: null, light: false, replay: null }).filter(hit => hit.kind === "execution");
