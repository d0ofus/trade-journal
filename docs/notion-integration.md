...
New pages use the registered template once. The job persists a unique creation title before sending the request and reconciles a lost response by that title and connection creator identity. It never adopts a manually created page or blindly repeats an unconfirmed page/section/content append. Template processing is shown as waiting, not a content conflict: checks follow 2, 4, 8, then 10-second delays for up to two minutes, respecting provider cooldowns. A timeout pauses for explicit Resume on the same page; the app does not reapply the template or erase page content. Partial/mismatched structure is never written into. Permission failures, uncertain writes and content conflicts still require attention rather than automatic retries.

Opening **Publish/update in Notion** first flushes the app save coordinator, then automatically prepares the latest saved-review preview. **Confirm publish/update** is still required before any Notion write. The dialog shows saved, frozen-job and last-published revisions plus the linked page. An unfinished job takes precedence over newer previews; resuming never changes its frozen content. Once it succeeds, newer saved edits receive their own preview and confirmation. Opening an unchanged completed review does not republish it. Closing the dialog or navigating cancels pending retry timers and ignores late responses, while accepted server work remains durable.

Linking is `NotionPublication.groupKey` (stable application trade ID) to `pageId` (Notion page UUID), with section/block and image-upload mappings. Ticker-only page titles never merge different trades. Status responses include publication revision context and template-readiness phase; progress remains in existing JSON/retry fields, with no additional migration or Worker.

**Shift + J** is the configurable **Show/hide Journal** command in Keyboard shortcuts. It preserves the current normal/Focus/fullscreen mode and temporary sidebar sizes. In custom tabbed layouts it first activates an obscured Journal tab and, when hiding it, activates a sibling without hiding their shared group. Mobile uses its existing Journal view. Text entry and nested dialogs retain their normal keys. Existing explicit bindings take priority over the new default.

Reliability follow-up validation (22 September 2026): focused publication/client/API and isolated PostgreSQL tests, workstation/provider regressions, persistence/export tests, six shortcut browser scenarios and the fullscreen journal/capture regression passed. Focused TypeScript, changed-file lint, current-tree repository safety and the production build also passed. The authenticated publication browser scenario was added but remains unverified: the development server returned 404 for `/api/auth/csrf`, and launching a production-mode test server was blocked by the execution environment. These checks did not perform a live Notion publication, deployment, migration or production push; production delivery is separately authorized.
