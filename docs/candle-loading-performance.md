...
During the September 14 historical rollout, production responses exposed `syd1::iad1` in `x-vercel-id`, showing Sydney ingress and execution in Washington. The Neon endpoint is in `ap-southeast-2` (Sydney). The first full 200-second administrative run completed 13 jobs and advanced 25 provider fills; this mixed-cache workload is an operational observation, not a controlled benchmark. Cross-region database round trips are an avoidable source of latency.

`vercel.json` now sets the application's single function region to `syd1`, colocating database consumers and scheduled workers with Neon. The existing cron schedules, provider/feed, database endpoints, two-worker limit, rate slots, storage guards and `maxDuration = 300` remain in effect. The durable queue and fenced writes allow work to resume across deployments. Verify the production response region after each region-related deployment. See [Vercel function regions](https://vercel.com/docs/functions/configuring-functions/region), [Vercel response headers](https://vercel.com/docs/headers/response-headers#x-vercel-id) and the [Alpaca stock-bars pagination contract](https://docs.alpaca.markets/us/reference/stockbars).
