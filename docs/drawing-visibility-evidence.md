...
- The Drawings panel's Hide all switch snapshots the IDs of all current drawings for temporary hiding across the workspace, fullscreen, pin controls, and new captures. Choosing tools does not reveal that batch: new drawings, duplicates and placement previews remain visible, subject to chart/replay/Before entry restrictions. **Show all drawings** clears the batch without changing individual saved hidden flags. A later Hide all includes drawings created since the previous batch. Hidden IDs survive deletion and undo/redo during the session; switching trades or reloading resets only temporary hiding. Drawings, preferences, saved views, undo history and execution markers are not rewritten by the switch.
...
Verified locally on 2026-09-23 for batched drawing visibility: 69 workstation unit tests, 118 mocked provider/API tests, and 16 local preview browser tests passed. Coverage includes mixed old/new visibility, placement previews, duplication, deletion/undo/redo, individual hidden flags, PNG captures, multiple charts, pins, fullscreen, replay/Before entry, reload and trade switching. Two-point placement previews now survive release of the first click. Focused TypeScript, relevant ESLint, current-file repository safety and the production build passed. Browser tests used synthetic demo storage with API requests blocked; no authenticated/production database, deployment or live-provider validation was performed for this change. No commit, push, migration or Worker deployment was performed.
